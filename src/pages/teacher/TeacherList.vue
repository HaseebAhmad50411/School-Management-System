  ...
  <q-dialog v-model="editDialog">
    <q-card>
      <q-card-section>
        <div class="row">
          <div class="col-12">
            <q-input label="Name" required v-model="row.name" />
          </div>
          <div class="col-12">
            <q-input label="Email" required v-model="row.email" />
          </div>
          <div class="col-12">
            <q-file
              filled
...
export default defineComponent({
  ...
  methods: {
    ...
    createNewTeacher() {
      this.$router.push({ path: "/teacher_form" });
    },
    async fetchSubjects() {
      try {
    ...
    async saveUpdate() {
      const rowId = this.row.id;
      try {
        const res = await api("put", `teachers/${rowId}`, this.row, {
          headers: {
    ...
    async uploadImage() {
      ...
      try {
        ...
        this.row.img_id = res?.data?.image?.id || null;
